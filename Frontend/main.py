...
def tutorial():
    st.subheader("IoVision")
    st.write(f"""
         This application works as google search do. With IoVision wich is a context and text based image retrieval web application. 
            \n You can search similar images by using a key word or sentance, by provinding an another image, by provinding another image
            by directly using its link and finally you can search similar images using a query image and text query (Ex. butterflies image + Orange as the text query = Orange butterflies).
            \n In addition, you can do recursive search by clicking on an image from the results.
    ...
    st.markdown(
                    f""" We are [Malek Abbes](https://github.com/Malek-1) and [Ines Boughariou](https://github.com/Ines38) a final year ICT engineering students
    ...
    st.markdown(f""" We have used the [open images dataset](https://github.com/cvdfoundation/open-images-dataset#download-full-dataset-with-google-storage-transfer) provided by google.
                   ...
                   # \n If you want to reproduce the result you can visit the github [repo](https://github.com/Ines38/CBIR-Search-Engine)
                    """)

def init_view():
    if st.session_state.search_term is None:
        text_exp = st.sidebar.expander("Text-based search", False)
    else:
        text_exp = st.sidebar.expander("Text-based search", True)
    ...
    with text_exp:
        ...
        search_fields = st.multiselect("Select search fields", options=["Title","Tags"], help='Enter the fields to search in')
    ...
    img_exp = st.sidebar.expander("Image-based search")
    with img_exp:
...
def text_search_query(search_res_numbers, text_search, use_fuzzy, fields,from_):
    ...
    fields = '{"tags":1, "Title":2}' if fields == {} or fields == '}' else fields
    ...
    if total_hits > 0:
        ...
        for i in range(len(results['hits']['hits'])):
            ...
            res = {}
            res["url"] = result['_source']["OriginalURL"]
...
def main():
    st.set_page_config(page_title='IoVision Search Engine')
    
    set_session_state()
    
    st.title("IoVision Search Engine")
